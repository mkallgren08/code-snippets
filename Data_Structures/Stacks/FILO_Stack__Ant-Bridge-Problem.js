...
// This is an example of a FILO (First in, last out) stack structure

const antBridge = (ants, terrain) => {
  let arr=ants.split('');
  for(let i=0;i<terrain.length;i++){
    if(terrain[i]==='.' || terrain[i-1]==='.' || terrain[i+1]==='.'){
      // Note: unshift adds an element to the beginning the array, pop removes the last element of the array
      // the code below is saying "take the last element of the array and add it to the beginning"
      arr.unshift(arr.pop())
    }
  }
...
antBridge("GFEDCBA", "------------...-----------") // should return "EDCBAGF"
antBridge("GFEDCBA", "-----------------------") // should return "GFEDCBA"
antBridge("EDCBA","-..------...------...---") // should return DCBAE